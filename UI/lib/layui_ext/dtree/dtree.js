...
layui.define(['jquery','layer','element','form'], function(exports) {
	...
	var DTree = function(options){
		...
		this.style = style;
		this.self_extend = self_extend; // new add by jjh on 2018-11-30
	...
	DTree.prototype.setting = function(options) {
		...
		/** 扩展参数 extend**/
		this.self_extend = this.options.self_extend || {scene: "common"}; // 扩展参数场景值，默认为common
	...
	// 加载toolBar中的内容
	DTree.prototype.loadToolBar = function(title, name,node){	
		var _this = this;
		var toolbarShow = _this.toolbarShow;
		...
		switch (name) {
			case eventName.addToolbar:
				...
				//1. 必须加载的节点内容
				var nowNode = ['<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">当前选中：</label>',
				...
				var addNodeType = ['<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">'+_this.toolbarStyle.title + '类型：</label>',
								'<div class="layui-input-block f-input-par">',
								'<input type="radio" name="departType" value="class" title="普通类目" checked lay-filter="departTypeFil">',
								'<input type="radio" name="departType" value="shop" title="门店" lay-filter="departTypeFil">',
								'<input type="radio" name="departType" value="cashwin" title="收银窗口" lay-filter="departTypeFil">',
								'</div>',
								'</div>'].join('');
				
				var addNodeName = ['<div class="layui-form-item" style="color:#000000">',
				               '<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">新增'+_this.toolbarStyle.title+'：</label>',
				               '<div class="layui-input-block f-input-par">',
				               '<input type="text" name="addNodeName" class="layui-input f-input-next-clear f-input" value="" lay-verify="required"/>',
				               '</div>',
				               '</div>'].join('');
				               
				var radioShowDiv = ['<div id="shopRadioShow" style="display: none;">',
								'<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">主动撤单：</label>',
								'<div class="layui-input-block f-input-par">',
								'<select name="cancelOrder">',
								...
								'</div>',
								'<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">口碑商店：</label>',
								'<div class="layui-input-block f-input-par">',
								'<input type="text" name="diExtend" class="layui-input f-input-next-clear f-input" value="">',
								'</div>',
								'</div>',
								'</div>'].join('');
				...
				var addNodeBtn = ['<div class="layui-form-item">',
					               ...
					               '</div>'].join('');
				//2. 用户自定义的节点内容
				if(_this.self_extend.scene === "department"){
					var addNodeBar = ['<div class="nav-tree-toolbar-div-tool"><form class="layui-form layui-form-pane" lay-filter="dtree_addNode_form">', nowNode, addNodeType, addNodeName,radioShowDiv];
				}else {
			...
			case eventName.editToolbar:
				...
				if(_this.self_extend.scene === "department"){
					var editNodeType = ['<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">'+_this.toolbarStyle.title + '类型：</label>',
								'<div class="layui-input-block f-input-par">',
								'<input type="radio" name="departType" value="class" title="普通类目" checked lay-filter="departTypeFil">',
								'<input type="radio" name="departType" value="shop" title="门店" lay-filter="departTypeFil">',
				...
				var editNodeName = ['<div class="layui-form-item" style="color:#000000">',
				...
				if(_this.self_extend.scene === "department"){
					var basicData = {};
					if(node.basicData){
						basicData = JSON.parse(node.basicData);
					}
					var editRadioShowDiv = ['<div id="shopRadioShow" style="display: none;">',
								'<div class="layui-form-item" style="color:#000000">',
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">主动撤单：</label>',
								'<div class="layui-input-block f-input-par">',
								'<select name="cancelOrder">',
								...
								'<label class="layui-form-label" style="background-color:#2A58AD;color:#ffffff">口碑商店：</label>',
								'<div class="layui-input-block f-input-par">',
//								'<input type="text" name="diExtend" class="layui-input f-input-next-clear f-input" value="' + (basicData.diExtend?basicData.diExtend:'') + '" lay-verify="required">',
								'<input type="text" name="diExtend" class="layui-input f-input-next-clear f-input" value="">',
								'</div>',
								'</div>',
								'</div>'].join('');
				...
				if(_this.self_extend.scene === "department"){
					var editNodeBar = ['<div class="nav-tree-toolbar-div-tool"><form class="layui-form layui-form-pane" lay-filter="dtree_editNode_form">', nowNode,editNodeType, editNodeName,editRadioShowDiv];
				}else {
	...
	DTree.prototype.bindBrowserEvent = function(nodes){
		...
		_this.obj.prev('div#dtree_toolbar_'+_this.obj[0].id).on("click", "a[d-click='"+eventName.addToolbar+"']", function(event) {
			...
			var $div = _this.getNowNode(),
				node = _this.getNodeParam($div),  // 当前节点的node数据信息
				$ul = $div.next("ul"),
				$p_li = $div.parent("li[data-index]"),	//当前选中节点的顶级li节点
			...
			var content = _this.loadToolBar(title, eventName.addToolbar,node);
			...
			layer.open({
				...
				success: function(layero, index){
					if(_this.self_extend.scene === "department"){
						form.render();
						form.on('radio(departTypeFil)', function(data){
						  if(data.value === "shop"){
						  	$("#shopRadioShow").show();
						  }else {
						  	$("#shopRadioShow").hide();
		...
		_this.obj.prev('div#dtree_toolbar_'+_this.obj[0].id).on("click", "a[d-click='"+eventName.editToolbar+"']", function(event) {
			...
			var content = _this.loadToolBar(title, eventName.editToolbar,node);
						...
						if(node.basicData){
							...
							if(basicData){
								form.val("dtree_editNode_form",{
									"departType": basicData.diType,
									"cancelOrder": basicData.diCancelOrder,
									"diExtend": basicData.diExtend
								});
							
								if(basicData.diType === "shop"){
									$("#shopRadioShow").show();
								}
							}
						}
